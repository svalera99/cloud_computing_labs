...
resource "google_storage_bucket_object" "db_exporter" {
  name   = "bucket-exporter-source.zip"
  bucket = google_storage_bucket.bucket.name
  source = "../../functions/bucket-exporter-source.zip"
}

resource "google_cloudfunctions2_function" "db-exporter-function" {
  name        = "my-db-exporter-function"
  location    = var.region.region
  description = "FaaS"

  build_config {
    runtime     = "python310"
    entry_point = "export_firestore"
    environment_variables = {
      BUILD_CONFIG_TEST = "build_test"
    }
    source {
      storage_source {
        bucket = google_storage_bucket.bucket.name
        object = google_storage_bucket_object.db_exporter.name
      }
    }
  }
